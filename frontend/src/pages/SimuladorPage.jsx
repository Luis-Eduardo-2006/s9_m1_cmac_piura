...
export default function SimuladorPage() {
  ...
  const input = { width: '100%', padding: '0.65rem 0.8rem', border: '1.5px solid #E0E6F0', borderRadius: 10, fontSize: '0.95rem', boxSizing: 'border-box', outline: 'none', fontFamily: 'Inter, sans-serif' };
  const th    = { textAlign: 'right', padding: '0.6rem 0.8rem', fontSize: '0.73rem', color: '#fff', fontWeight: 700, letterSpacing: '0.3px' };
  const td    = { textAlign: 'right', padding: '0.5rem 0.8rem', fontSize: '0.82rem', borderBottom: '1px solid #F0F4FA', color: '#333' };

  return (
    <div style={{ fontFamily: 'Inter, sans-serif', background: '#EEF2FA', minHeight: '100vh' }}>

      {/* Header */}
      <div style={{ background: 'linear-gradient(120deg, #003580 0%, #004A9F 60%, #0060C8 100%)', padding: '1.5rem 2rem 3rem', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 1000, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
            <div style={{ width: 48, height: 48, background: AMARILLO, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <i className="fa-solid fa-calculator" style={{ color: '#fff', fontSize: '1.3rem' }} />
            </div>
            <div>
              <h1 style={{ color: '#fff', margin: 0, fontSize: '1.6rem', fontWeight: 900 }}>Simulador de crédito</h1>
      ...
      <div style={{ maxWidth: 1000, margin: '-1.5rem auto 0', padding: '0 2rem 3rem', position: 'relative', zIndex: 2 }}>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.2rem', alignItems: 'start', marginBottom: '1.4rem' }}>
          ...
          <div style={card}>
            ...
            <select style={{ ...input, marginBottom: '1.2rem', cursor: 'pointer' }} value={casoCargado} onChange={(e) => cargarCaso(e.target.value)}>
            ...
            <input style={{ ...input, marginBottom: '1.2rem' }} type="number" min="1" value={monto}
              onChange={(e) => { setMonto(e.target.value); setCasoCargado(''); }} />

            <label style={label}>Plazo (meses)</label>
            <input style={{ ...input, marginBottom: '1.2rem' }} type="number" min="1" value={meses}
              onChange={(e) => { setMeses(e.target.value); setCasoCargado(''); }} />

            <label style={label}>Primera fecha de pago</label>
            <input style={{ ...input, marginBottom: '1.2rem' }} type="date" value={primeraCuota}
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: '0.88rem', color: '#333', cursor: 'pointer', padding: '0.7rem 0.8rem', background: conDesgravamen ? '#EEF5FF' : '#F8F9FC', borderRadius: 10, border: `1.5px solid ${conDesgravamen ? AZUL : '#E0E6F0'}` }}>
              <input type="checkbox" checked={conDesgravamen} onChange={(e) => { setConDesgravamen(e.target.checked); setCasoCargado(''); }}
                style={{ width: 16, height: 16, accentColor: AZUL }} />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
            ...
            <div style={{ ...card, background: 'linear-gradient(120deg, #003580 0%, #004A9F 100%)', color: '#fff', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ fontSize: '2.4rem', fontWeight: 900, color: AMARILLO, letterSpacing: '-1px', lineHeight: 1 }}>
            ...
            {casoRef && resultado && (
              <div style={{
                padding: '0.8rem 1rem', borderRadius: 12, fontSize: '0.82rem',
                background: coincide ? '#E8F8F0' : '#FEF0F0',
                border: `1.5px solid ${coincide ? '#27AE60' : '#E74C3C'}`,
                color: coincide ? '#1b7a3d' : '#b3261e',
        ...
        {resultado && (
          <div style={{ ...card, padding: 0, overflow: 'hidden' }}>
            <div style={{ padding: '1rem 1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #F0F4FA' }}>
                ...
                <tbody>
                  {resultado.filas.map((f, i) => (
                    <tr key={f.n} style={{ background: i % 2 === 0 ? '#fff' : '#FAFBFF' }}>
                      ...
                      <td style={{ ...td, textAlign: 'left', color: '#666' }}>{f.fecha}</td>
