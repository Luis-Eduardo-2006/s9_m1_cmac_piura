    ...
    <div style={{ minHeight: '100vh', background: '#EEF2FA', fontFamily: "'Inter', sans-serif" }}>
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.55rem', cursor: 'pointer' }} onClick={() => navigate('/')}>
          <i className="fa-solid fa-building-columns" style={{ color: '#F5C200', fontSize: '1.3rem' }} />
          {!isMobile && <span style={{ color: '#fff', fontWeight: 800, fontSize: '1.05rem', letterSpacing: '0.5px' }}>CAJA PIURA</span>}
        </div>

        <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '0.7rem' : '1.1rem' }}>
          ...
          {!isMobile && <div style={{ width: '1px', height: '28px', background: 'rgba(255,255,255,0.2)' }} />}

          <div style={{ display: 'flex', alignItems: 'center', gap: '0.65rem' }}>
            <div style={{ width: '36px', height: '36px', background: 'linear-gradient(135deg, #F5C200, #e0ac00)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, color: '#003580', fontSize: '0.95rem', flexShrink: 0 }}>
              {inicial}
            </div>
            {!isMobile && (
              <div>
          ...
          <button onClick={handleLogout} style={{ background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.25)', color: '#fff', padding: isMobile ? '0.4rem 0.7rem' : '0.42rem 1rem', borderRadius: '8px', fontSize: '0.8rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.4rem', transition: 'background 0.2s' }}
      ...
      <section style={{
        ...
        color: '#fff',
        padding: isMobile ? '1.8rem 1rem 3.5rem' : '2.8rem 2rem 4rem',
        ...
        <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'flex', flexDirection: isSmall ? 'column' : 'row', justifyContent: 'space-between', alignItems: isSmall ? 'flex-start' : 'center', position: 'relative', zIndex: 1, gap: '1.5rem' }}>
          <div>
            ...
            <h1 style={{ fontSize: isMobile ? '1.6rem' : '2rem', fontWeight: 900, lineHeight: 1.2, marginBottom: '0.3rem' }}>
          ...
          {/* Glassmorphism total */}
          <div style={{ background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.18)', borderRadius: '18px', padding: isMobile ? '1rem 1.5rem' : '1.4rem 2rem', textAlign: 'center', width: isSmall ? '100%' : 'auto', minWidth: isSmall ? 'unset' : '210px' }}>
            ...
            {cargando
              ...
              : <p style={{ fontSize: isMobile ? '1.6rem' : '2rem', fontWeight: 900, color: '#F5C200', letterSpacing: '-0.5px' }}>{formatoSoles(totalSaldo)}</p>
            }
      ...
      {/* ── Contenido ── */}
      <div style={{ maxWidth: '1100px', margin: '-1.8rem auto 0', padding: isMobile ? '0 1rem 2rem' : '0 2rem 3rem', position: 'relative', zIndex: 2 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem', marginBottom: '1.4rem' }}>
          ...
          {!cargando && tarjetas.map((t) => (
            <div key={t.id} style={{ background: '#fff', borderRadius: '16px', padding: '1.3rem', boxShadow: '0 4px 20px rgba(0,74,159,0.07)', borderTop: `4px solid ${t.color}`, cursor: 'pointer', transition: 'transform 0.2s, box-shadow 0.2s', position: 'relative', overflow: 'hidden' }}
            ...
            >
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                ...
                <span style={{ background: `${t.color}15`, color: t.color, fontSize: '0.65rem', fontWeight: 700, padding: '0.18rem 0.55rem', borderRadius: '20px', textTransform: 'uppercase', letterSpacing: '0.4px' }}>Activa</span>
              </div>
              <p style={{ fontSize: '0.7rem', color: '#aaa', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '0.3rem' }}>{t.label}</p>
              <p style={{ fontSize: '1.45rem', fontWeight: 900, color: '#1A1A2E', letterSpacing: '-0.5px', marginBottom: '0.15rem' }}>{formatoSoles(t.saldo)}</p>
        ...
        {/* ── Grid principal ── */}
        <div style={{ display: 'grid', gridTemplateColumns: isSmall ? '1fr' : '1fr 295px', gap: '1.5rem', alignItems: 'start' }}>

          {/* Movimientos */}
          <div style={{ background: '#fff', borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,74,159,0.07)', overflow: 'hidden' }}>
            <div style={{ padding: '1.2rem 1.4rem', borderBottom: '1px solid #F0F4FA', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h2 style={{ fontSize: '0.95rem', fontWeight: 700, color: '#1A1A2E', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            <div>
              ...
              {movimientos.map((m, i) => (
                <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: '0.8rem', padding: isMobile ? '0.85rem 1rem' : '1rem 1.4rem', borderBottom: i < movimientos.length - 1 ? '1px solid #F7F9FC' : 'none', transition: 'background 0.15s', cursor: 'default' }}
                  onMouseEnter={e => e.currentTarget.style.background = '#F7F9FD'}
                  onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
                >
                  <div style={{ width: '40px', height: '40px', flexShrink: 0, background: m.tipo === 'entrada' ? '#E8F8F0' : '#FEF0F0', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <i className={`fa-solid ${m.tipo === 'entrada' ? 'fa-arrow-down' : 'fa-arrow-up'}`} style={{ color: m.tipo === 'entrada' ? '#27AE60' : '#E74C3C', fontSize: '0.88rem' }} />
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: '0.87rem', fontWeight: 600, color: '#1A1A2E', marginBottom: '0.1rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{m.descripcion}</p>
                    <p style={{ fontSize: '0.71rem', color: '#bbb' }}>{formatoFecha(m.fecha)}</p>
                  </div>
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <p style={{ fontSize: isMobile ? '0.82rem' : '0.92rem', fontWeight: 800, color: m.tipo === 'entrada' ? '#27AE60' : '#E74C3C', marginBottom: '0.15rem' }}>
                      {m.tipo === 'entrada' ? '+' : '-'}{formatoSoles(m.monto)}
                    </p>
                    <span style={{ fontSize: '0.63rem', fontWeight: 700, color: m.tipo === 'entrada' ? '#27AE60' : '#E74C3C', background: m.tipo === 'entrada' ? '#E8F8F0' : '#FEF0F0', padding: '0.1rem 0.45rem', borderRadius: '6px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
            ...
            {/* Accesos rápidos */}
            <div style={{ background: '#fff', borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,74,159,0.07)', padding: '1.2rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.6rem' }}>
                {acciones.map((a) => (
                  <button key={a.label} style={{ background: '#F5F7FA', border: '1.5px solid #E8EEF8', borderRadius: '12px', padding: '0.9rem 0.3rem', fontSize: '0.7rem', color: '#333', fontWeight: 600, cursor: 'pointer', textAlign: 'center', transition: 'all 0.2s' }}
                    onMouseEnter={e => { e.currentTarget.style.background = '#004A9F'; e.currentTarget.style.color = '#fff'; e.currentTarget.style.borderColor = '#004A9F'; e.currentTarget.style.transform = 'translateY(-3px)'; e.currentTarget.style.boxShadow = '0 8px 20px rgba(0,74,159,0.22)'; }}
                    onMouseLeave={e => { e.currentTarget.style.background = '#F5F7FA'; e.currentTarget.style.color = '#333'; e.currentTarget.style.borderColor = '#E8EEF8'; e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = 'none'; }}
                  >
                    <i className={`fa-solid ${a.icono}`} style={{ display: 'block', fontSize: '1.15rem', marginBottom: '0.35rem' }} />
                    {a.label}
                  </button>
                ))}
              </div>
            ...
            {/* Perfil */}
            <div style={{ background: '#fff', borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,74,159,0.07)', overflow: 'hidden' }}>
              <div style={{ background: 'linear-gradient(120deg, #003580 0%, #004A9F 100%)', padding: '1.2rem', display: 'flex', alignItems: 'center', gap: '0.85rem' }}>
                <div style={{ width: '46px', height: '46px', background: 'linear-gradient(135deg, #F5C200, #e0ac00)', borderRadius: '50%', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, fontSize: '1.15rem', color: '#003580' }}>
                  {inicial}
                </div>
                <div>
                  <div style={{ fontWeight: 700, color: '#fff', fontSize: '0.88rem', marginBottom: '0.2rem' }}>{nombre}</div>
                  <span style={{ fontSize: '0.66rem', fontWeight: 700, background: 'rgba(245,194,0,0.2)', color: '#F5C200', padding: '0.15rem 0.55rem', borderRadius: '10px' }}>Cliente MYPE</span>
                </div>
              </div>
            ...
            {/* Banner seguridad */}
            <div style={{ background: 'linear-gradient(120deg, #003580 0%, #004A9F 100%)', borderRadius: '16px', padding: '1.2rem', color: '#fff', position: 'relative', overflow: 'hidden' }}>
