...
export default function BancaPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#F5F7FA' }}>
      ...
      {/* Header */}
      <nav style={{ background: '#fff', borderBottom: '1px solid #E0E6EF', padding: isMobile ? '0.75rem 1rem' : '0.9rem 2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
        <img
        ...
        <span style={{ color: '#666', fontSize: '0.85rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.4rem' }} onClick={() => navigate('/')}>
          <i className="fa-solid fa-arrow-left" style={{ color: '#004A9F' }} />
          {!isMobile && ' Volver al inicio'}
        </span>
      </nav>

      {/* Contenido */}
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: isMobile ? '1.5rem 1rem' : '3rem 2rem' }}>

        <div style={{ background: '#fff', borderRadius: '16px', padding: isMobile ? '2rem 1.2rem' : '3rem 2.5rem', maxWidth: '500px', width: '100%', boxShadow: '0 8px 32px rgba(0,74,159,0.10)', textAlign: 'center' }}>

          <div style={{ width: '72px', height: '72px', background: 'linear-gradient(135deg, #004A9F, #0060C8)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1.5rem' }}>
            <i className="fa-solid fa-lock" style={{ color: '#fff', fontSize: '1.7rem' }} />
          </div>

          <h1 style={{ fontSize: isMobile ? '1.4rem' : '1.6rem', fontWeight: 800, color: '#004A9F', marginBottom: '0.5rem' }}>
            Caja Digital
          </h1>
        ...
        <div style={{ marginTop: '1.5rem', maxWidth: '500px', width: '100%', background: '#FFFBEB', border: '1px solid #F5C518', borderRadius: '10px', padding: '1rem 1.2rem', display: 'flex', gap: '0.7rem', alignItems: 'flex-start' }}>
          <i className="fa-solid fa-triangle-exclamation" style={{ color: '#D4A000', fontSize: '1rem', marginTop: '0.1rem', flexShrink: 0 }} />
